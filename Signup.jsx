...
const Register = () => {
  ...
  const validateName = (name) => name.length >= 3 && /^[A-Za-z\s]+$/.test(name);
  ...
  const handleInputChange = (setter, validator, fieldName) => (e) => {
    ...
    if (fieldName === "email") value = value.toLowerCase();
    setter(value);
  ...
  const getErrorMessage = (fieldName, value) => {
    const messages = {
      name: value?.length < 3 ? "Name must be at least 3 characters." : "Name must contain only letters and spaces.",
      email: "Invalid email format or uppercase letters are not allowed.",
  ...
  const handleSubmit = async (e) => {
    ...
    const url = isSignUp ? 'http://localhost:5000/api/public/register' : 'http://localhost:5000/api/public/login';
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (isSignUp) {
          ...
          if (user && user._id) sessionStorage.setItem("loggedInUser", user._id);
        ...
        <div className="form-container sign-up">
          <form onSubmit={handleSubmit}>
            ...
            <input
              ...
              onChange={handleInputChange(setConfirmPassword, (val) => validatePasswordMatch(password, val), "confirmPassword")}
              style={{ borderColor: errors.confirmPassword ? "red" : "" }}
            />
            {errors.confirmPassword && <small className="error-message">{errors.confirmPassword}</small>}

            <div className="role-toggle">
              <label className={`role-label ${role === 'user' ? 'active' : ''}`}>User</label>
              <div className="toggle-switch" onClick={() => setRole(role === 'user' ? 'veterinarian' : 'user')}>
                <span className={`toggle-circle ${role}`}></span>
              </div>
              <label className={`role-label ${role === 'veterinarian' ? 'active' : ''}`}>Veterinarian</label>
            </div>
